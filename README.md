...
| MBBank | ⏳ Chưa thêm được | Trang này là ứng dụng JavaScript thuần (SPA), không thể đọc được bằng cách quét HTML thông thường. Cần tìm được link API dữ liệu (JSON) qua tab Network của trình duyệt để bot có thể đọc được. Xem hướng dẫn chi tiết bên dưới. |

### Cách lấy link API cho MBBank (và các website JS-only khác sau này)
